...
export class FriendsGateway implements OnGatewayConnection, OnGatewayDisconnect {
	...
	handleJoinRelation(@ConnectedSocket() client: Socket, @MessageBody() data: { userId: number }) {
	...
	async handleUnsendRequest(
		...
		const result = await this.friendsService.cancelRequest(data.requesterId, data.addresseId);
	...
	async handleRemoveFriend(
		...
		const result = await this.friendsService.removeFriend(data.userId, data.friendId);
	...
	async handleBlockFriend(
		...
		const result = await this.friendsService.blockFriend(data.userId, data.targetId);
	...
	async handleUnblockFriend(
		...
		const result = await this.friendsService.unBlockFriend(data.userId, data.targetId);
