...
export function ChatWindow({ conversationId, currentUserId, otherUser, onNewMessage }: Props) {
	...
	useEffect(() => {
		otherUserIdRef.current = otherUser?.id;
	}, [otherUser?.id]);
	const { t } = useTranslation();

	const { emit } = useFriendsSocket(user?.id, {
		onUserOnline: (data) => {
			if (data.userId === otherUserIdRef.current) setIsOnline(true);
		},
		onUserOffline: (data) => {
			if (data.userId === otherUserIdRef.current) setIsOnline(false);
		},
		onOnlineStatus: (data) => {
			if (data.userId === otherUserIdRef.current) setIsOnline(data.isOnline);
		},
	});
			...
			>
				{(messages as Message[]).map((msg, i) => {
					...
					const bubbleBg = isOwn
						? isTournamentVictory || isTournamentInvite
							? 'rgba(249, 216, 230, 0.8)'
							: '#6b7280'
						: '#f3f4f6';
									...
									>
										<div
											style={{
												...
												color:
													isOwn &&
													(isTournamentInvite || isTournamentVictory)
														? '#b05070'
														: isOwn
															? '#fff'
															: '#111827',
