...
export class JwtAuthGuard extends AuthGuard('jwt') {
	...
	canActivate(context: ExecutionContext) {
		const isPublic = this.reflector.getAllAndOverride<boolean>(IS_PUBLIC_KEY, [ //checks if metadata exists for the given key first on the handler, then on the class.
			context.getHandler(),
			context.getClass(),
		]);
